...
## Иерархия: 2 реальных уровня + расширяемость

**По результатам исследования (23-SITUATIONS-SPECIFICITY.md):** 4-уровневая иерархия — фикция. Реально уникальный контент существует только на 2 уровнях:

```
Universal (764 ситуации, 36K DTC) ← базовые правила для всех
  └→ Brand (58 брендов, до 1233 ситуаций) ← реальные болячки бренда
      └→ Model = ФИЛЬТР brand (0% уникального контента, подмножество)
          └→ Generation = пока только мануальные выдержки (не диагностика)
```

**Факты:**
- Model-level: 100% записей есть в brand-level с идентичным текстом. Просто фильтрация по полю `models`.
- Generation "уникальные" 1195 записей — выдержки из сервисных мануалов, не адаптированные диагностические ситуации.
- 443 ID общие для BMW X5 E53/E70/F15 — 100% идентичный quickAnswer, title, facts_ru, priceData. Ноль адаптации.

**Решение:** Реализуем resolver на 2 уровнях (universal + brand). Архитектура ПОДДЕРЖИВАЕТ 4 уровня — когда мы сами напишем generation-specific overlays с конкретными двигателями, ценами и типичными проблемами по поколениям.

**Принцип:** most specific wins. Ищем от самого специфичного к общему. Первый найденный — используем.
