...
- Стадия I (одна гармоника) не диагностируется по 10 пикам FFT — нужен envelope analysis (A.9, A.30)

**Правила:** 1.3; roadmap через A.30 (SANC → Kurtogram → Envelope) для Stage I детекции
...
**Применение в LLCAR:**
Полный envelope analysis не реализован — требует raw audio (44.1 кГц). Текущий proxy: 10 дискретных FFT-пиков аудио + bpfo_harmonic_matches. Будущая реализация: полный pipeline **SANC → Spectral Kurtosis (A.26) → Fast Kurtogram → bandpass/WPT → Hilbert → envelope FFT** описан в A.30 (Randall & Antoni 2011). Альтернатива предобработки через VMD (K=6) → выбор информативной моды по kurtosis → Hilbert → envelope FFT.
...
**Применение в LLCAR:**
az_shape4 (4-й момент нормированного распределения az) используется как proxy kurtosis. Порог K > 5 в сочетании с crest_factor_z > 4 → признак импульсного дефекта. Различение детонации (kurtosis > 6, частота > 4 кГц) от стука подвески (kurtosis 4–6, частота < 500 Гц). Частотно-разрешённая версия kurtosis описана в **A.26 Spectral Kurtosis** — SK(f) как 4-й стандартизированный момент STFT-коэффициентов позволяет локализовать импульсность в конкретной полосе (используется в shadow-правиле 6.4 spectral_kurtosis_impulsive_bearing).
...
Spectral Kurtosis (SK) — частотно-разрешённая версия классического эксцесса (A.12), измеряющая импульсность (тяжесть хвостов распределения) отдельно в каждой частотной полосе. Метод введён Antoni (2006) как развитие идеи Dwyer (1983): строгое статистическое определение SK(f) как 4-го стандартизированного момента STFT-коэффициентов. Главное преимущество SK над временным kurtosis: при маскирующем гауссовом шуме и узкополосном резонансе SK показывает, **в какой именно полосе частот** скрыты импульсы дефекта. Antoni (2007) предложил Fast Kurtogram — древовидный банк фильтров (filter bank), вычисляющий SK(f, Δf) на дискретной сетке полос и масштабов, и возвращающий **оптимальную полосу [f_low, f_high]** с максимальным SK. Эта полоса затем используется для envelope analysis (A.9). Метод стал стандартом диагностики подшипников (Randall & Antoni 2011) и расширяется на детонацию ДВС, износ ГРМ и удары маслосъёмных колец. Для ступичного подшипника 6206 с дефектом наружного кольца kurtogram_best_band типично 2.5–4 кГц (первый структурный резонанс ступицы).

**Математическая основа:**

```
Определение Spectral Kurtosis через STFT X(t,f):
  SK(f) = ⟨|X(t,f)|⁴⟩_t / ⟨|X(t,f)|²⟩²_t  −  2

  ⟨·⟩_t = среднее по времени (по кадрам STFT)
  "−2" — нормировка: SK(f) = 0 для стационарного гауссового сигнала
  SK(f) > 0 — признак импульсного процесса в полосе f
  SK(f) < 0 — признак синусоидального (тонального) процесса

Связь с временным kurtosis A.12:
  K_temporal = SK(f), интегрированный по всей полосе при N=1 (один фильтр)
  SK(f) = частный случай kurtosis в отдельной полосе → выше разрешающая способность

Fast Kurtogram (Antoni 2007):
  Алгоритм tree-structured filter bank (1/3-binary tree):
    Level k=0: полная полоса [0, fs/2], один фильтр → SK₀
    Level k=1: две полосы [0, fs/4], [fs/4, fs/2] → SK₁,₁, SK₁,₂
    Level k=K: 2^k полос → 2^K значений SK
  Kurtogram(f, Δf) — 2D-матрица SK для всех уровней k=0..K и центральных частот f_c
  best_band = argmax(f_low, f_high) SK(f_c, Δf)

Статистическая значимость (Antoni 2006):
  Для стационарного гауссового процесса: SK ~ N(0, 4/N_frames)
  Порог «impulsive signal detected»: SK > 2·√(4/N_frames) = 4/√N_frames
  При N_frames=100: SK > 0.4; при N_frames=25: SK > 0.8
  Порог «severe impulsive bearing fault»: SK > 3–5
```

**Численные примеры — CWRU bearing dataset, outer race fault:**

```
Сигнал: подшипник SKF 6205 с дефектом 0.007" на наружном кольце, f_r=29 Гц,
        fs=12 кГц, длительность 10 сек, SNR через маскирование ~20 дБ шумом

Level k   Число полос   Ширина Δf   Максимум SK   best_band [Гц]
0         1             6000        0.3           [0, 6000]
1         2             3000        0.7           [3000, 6000]
2         4             1500        1.9           [3000, 4500]
3         8             750         3.1           [3000, 3750]   ← первый структурный резонанс
4         16            375         4.8           [3375, 3750]   ← максимум
5         32            187         4.2           [3375, 3562]
6         64            93          3.5           (переполнение, шум ядра)

После bandpass [3375, 3750] + envelope FFT:
  Пик 107 Гц ≈ BPFO (29·3.7 ≈ 107 Гц) — подтверждение
  Гармоники 2·BPFO, 3·BPFO, 4·BPFO видны чётко
  Sidebands ±f_r = ±29 Гц (модуляция ротационной частотой)
```

**Применение в LLCAR:**
Shadow-правило 6.4 (spectral_kurtosis_impulsive_bearing) использует `spectral_kurtosis_audio` — максимум SK(f) по STFT аудио в 500-мс окне, как ранний индикатор импульсного дефекта подшипника (Stage II–III) ещё до появления ≥3 гармоник BPFO в 10-пиковом FFT. После shadow-валидации (30+ дней, precision ≥ 0.6) правило продвигается в production 1.29. Для детонации ДВС (1.15) Fast Kurtogram заменит жёсткую полосу 5–8 кГц на динамически выбираемую kurtogram_best_band — это учитывает индивидуальные структурные резонансы конкретного блока (bore-зависимый, см. A.31 Draper).

**Ограничения:**
- SK чувствителен к циклостационарным несущим: ГРМ, поршневые импульсы дают ложно-высокие SK без дефекта подшипника → комбинация с bpfo_harmonic_matches обязательна
- При SNR < 0 дБ kurtogram может «зацепиться» за шумовой резонанс, а не за дефект
- Minimum N_frames ≥ 25 для статистической значимости → нужно окно ≥ 0.5 с при hop=20 мс
- Stage IV-парадокс kurtosis (A.12) действует и для SK: при полном разрушении подшипника SK снижается

**Правила:** 6.4 (shadow, spectral_kurtosis_impulsive_bearing), roadmap 1.29 (production); дополнение к 1.15 (knock_impulse_kurtogram_band)
...
- Antoni J. (2007). Fast computation of the kurtogram for the detection of transient faults. *Mechanical Systems and Signal Processing*, 21(1):108–124. DOI:10.1016/j.ymssp.2005.12.002
- Dwyer R.F. (1983). Detection of non-Gaussian signals by frequency domain kurtosis estimation. *Proc. ICASSP*, 8:607–610.
...
Order Tracking — техника частотного анализа вращающихся машин, переводящая спектр из абсолютных частот (Гц) в безразмерные порядки (orders) O = f·60/n, где n — обороты в минуту. Главное преимущество: при переменной частоте вращения (run-up, coast-down, изменение оборотов в пробке) гармоники двигателя смазываются в обычном FFT, но остаются фиксированными (O=1, 2, 3, …) в спектре порядков. Fyfe & Munck (1997) формализовали Computed Order Tracking (COT) — цифровой алгоритм resampling'а сигнала по углу поворота Δθ=const вместо Δt=const. Для L4-двигателя порядок 2 (f_fire = 2·f_c) соответствует вспышкам; порядок 1 — первичному дисбалансу; порядок 0.5 — пропускам (misfire, 1 вспышка за 2 оборота коленвала). Связь со спектром: при 2000 об/мин (f_c=33.3 Гц) order 2 = 66.7 Гц; при 3000 об/мин order 2 = 100 Гц, но в спектре порядков оба случая дают пик ровно на O=2. Комбинирование COT с LLCAR RPM через OBD (PID 0x0C) превращает FFT-пики в поток порядков, устойчивый к дрейфу оборотов.

**Математическая основа:**

```
Определение порядка:
  O = f · 60 / n    [безразмерный]
    f = частота сигнала [Гц]
    n = обороты в минуту [об/мин]
  Эквивалентно: O = f / f_c, где f_c = n/60 — частота вращения [Гц]

Computed Order Tracking (Fyfe & Munck 1997):
  Шаг 1: Запись синхронных сигналов: x(t_k) — вибрация/аудио; θ(t_k) — угол коленвала
         θ(t) интегрируется из tacho-пульсов или из OBD-RPM:
         θ(t) = 2π·∫n(τ)/60 dτ
  Шаг 2: Обратная интерполяция: t = t(θ) через сплайн-интерполяцию
  Шаг 3: Resampling: x_θ[k] = x(t(k·Δθ)), k=0..N-1
         Обычно Δθ = 2π/S, где S — число выборок на оборот (например, S=64)
  Шаг 4: FFT от x_θ[k] → спектр в порядках, Δorder = 1/(N/S) = S/N об⁻¹

Типичные порядки двигателя (f_c — частота коленвала):
  4-тактный, L4:   O = 2   (2 вспышки/оборот) — доминанта
  4-тактный, L6:   O = 3   (3 вспышки/оборот)
  4-тактный, V8:   O = 4   (4 вспышки/оборот)
  Дисбаланс:       O = 1   (1-й порядок, центробежная сила)
  Инерция 2-го рода (Lanchester): O = 2  (у L4 без балансирных валов)
  Misfire:         O = 0.5 (1 пропуск за 2 оборота) ← см. A.28
  ГРМ (timing):    O = 0.5 (полуоборотный ремень/цепь)
  Turbo:           O = 50–150 (n_turbo ≈ 50–150× n_engine)
```

**Численные примеры — L4-двигатель в разгоне 1000 → 3000 об/мин:**

```
RPM       f_c [Гц]   Order 2 (f) [Гц]   Order 0.5 [Гц]   Turbo Order 80 [Гц]
1000      16.7       33.3               8.3              1333
1500      25.0       50.0               12.5             2000
2000      33.3       66.7               16.7             2667
2500      41.7       83.3               20.8             3333
3000      50.0       100.0              25.0             4000

В обычном FFT 500 мс окна пик вспышек «размазан» по 33–100 Гц и не виден.
В спектре порядков O=2 остаётся одной узкой линией при любом RPM.
```

**Применение в LLCAR:**
Новая фича `fft_orders[0..9]` — массив пиков FFT-аудио, переведённых в порядки при rpm > 800 об/мин (OBD доступен): order_k = freq_k · 60 / rpm. Фича `rpm_order_matches` считает совпадения с целевым набором {0.5, 1.0, 2.0, 3.0, 4.0} ±0.05. Shadow-правило 6.5 (order_tracking_mount_wear_shadow) использует ≥3 совпадений как сигнал «чистая RPM-гармоника → источник двигатель, не подвеска». Это модификация правила 1.2 (engine_mount_harmonic) — переход с абсолютных частот на порядки снижает ложные тревоги при разном RPM между поездками. Для L4-двигателей без балансирных валов Lanchester аномально высокий order_2x_amp (z>2.5) сигнализирует отказ балансирного механизма (планируемое production-правило order_2x_imbalance_l4).

**Ограничения:**
- COT требует синхронный тахо-сигнал; LLCAR использует OBD RPM с частотой обновления 10–20 Гц → at run-up > 500 об/мин/с возможен spectral leakage
- Разрешение по порядку Δorder = S/N, при S=64, N=2048 Δorder ≈ 0.031 — достаточно для различения O=0.5 и O=1.0
- На очень низких RPM (< 800 об/мин) OBD может пропускать пакеты → order tracking отключается
- Перестановки зажигания (цикл 1-3-4-2) создают разные гармоники для L4 и L6; для V-образных моторов дополнительно появляются half-orders O=1.5, 2.5

**Правила:** 6.5 (shadow, order_tracking_mount_wear_shadow), 1.2 (модификация с order_matches), roadmap order_2x_imbalance_l4, roadmap order_05_misfire_diesel
...
- Antoni J. et al. (2004). The relationship between spectral correlation and envelope analysis in the diagnostics of bearings. *Mechanical Systems and Signal Processing*, 18(6):1285–1314. DOI:10.1016/S0888-3270(03)00088-8
- Randall R.B., Antoni J. (2011). Rolling element bearing diagnostics — A tutorial. *MSSP*, 25(2):485–520. DOI:10.1016/j.ymssp.2010.07.017

---

### A.28 Cyclostationary Modelling (циклостационарный анализ)

Циклостационарный сигнал — стохастический процесс, статистические характеристики которого (среднее, автокорреляция, спектр) периодичны с периодом T_cycle. Двигатель внутреннего сгорания — классический циклостационарный источник: рабочий цикл 4-тактного ДВС длится 2 оборота коленвала (T_cycle = 2/f_c), за это время в каждом цилиндре происходит одна вспышка. Antoni et al. (2004) показал, что envelope analysis (A.9) и Spectral Kurtosis (A.26) — частные случаи более общей циклостационарной теории: автокорреляция R_x(τ, α) в функции от временного сдвига τ и циклической частоты α содержит полную информацию о скрытых периодичностях. Для дизельного двигателя T_cycle = 2/f_c соответствует циклической частоте α₀ = f_c/2 = RPM/120. Пропуски зажигания (misfire) возбуждают дополнительную гармонику на α = f_c/2 = order 0.5 → Carlucci et al. (2006) качественно описали эту связь для дизеля. В LLCAR циклостационарный анализ используется как теоретическая рамка для Order Tracking (A.27) и spectral coherence между каналами ax/az.

**Математическая основа:**

```
Циклостационарная автокорреляция (Antoni 2004):
  R_x(τ, α) = ⟨x(t)·x*(t−τ)·exp(−j2παt)⟩_t
    τ = временной сдвиг
    α = циклическая частота (fundamental T_cycle и гармоники)

Спектральная корреляция (2D FFT):
  S_x(f, α) = FFT_τ { R_x(τ, α) }
    S_x(f, 0) = обычный PSD
    S_x(f, α≠0) ≠ 0  ⟺  циклостационарный процесс

Циклические частоты ДВС (4-тактный):
  T_cycle = 2 / f_c           (2 оборота коленвала на цикл)
  α_fire  = 1 / T_cycle · N_cyl = N_cyl · f_c / 2    (частота вспышек)
  α_misfire = 1 / T_cycle = f_c / 2   ← ключевой маркер пропуска

Связь с envelope analysis (A.9):
  При узкополосной модуляции fc_resonance амплитудой f_α:
    envelope spectrum показывает пик на α
    ⟺  цикличёская плотность S_x(f_resonance, α) значимо > 0
```

**Численные примеры — дизельный двигатель 4-цилиндровый, 1800 об/мин:**

```
f_c = 30 Гц (коленвал)
T_cycle = 2/30 = 66.7 мс (4-тактный цикл)
α_fire  = 4·30/2 = 60 Гц   (частота вспышек, O=2)
α_misfire = 30/2 = 15 Гц   (O=0.5)

Spectral Kurtosis в полосе 2–4 кГц (резонанс головки блока) показывает:
  Пик α = 60 Гц при здоровом двигателе  (циклостационарная модуляция вспышками)
  Дополнительный пик α = 15 Гц при misfire → качественно Carlucci 2006

ВАЖНО (атрибуция): Carlucci 2006 даёт только качественное описание
связи order 0.5 с misfire на дизеле. Конкретные амплитудные пороги
не извлечены из первоисточника — в LLCAR используется только
качественная логика «amp(0.5) > amp(2.0)·0.3 → подозрение на пропуск».
```

**Применение в LLCAR:**
Теоретическая рамка для Order Tracking (A.27) и shadow-правила order_05_misfire_diesel (условие: fuel_type=="diesel" + amp(0.5) > amp(2.0)·0.3). Полный spectral correlation S_x(f, α) не вычисляется в реальном времени из-за вычислительной стоимости O(N²); используется proxy через order spectrum. Cyclostationary framework также связывает Kurtogram (A.26) и envelope analysis (A.9) — обе техники реконструируют части S_x(f, α).

**Ограничения:**
- Полный spectral correlation требует N ≥ 2^14 выборок и O(N²) операций → 500-мс окна недостаточно
- LLCAR использует только proxy: order spectrum (1D-срез циклостационарности)
- Разделение misfire и нормальной вспышечной асимметрии требует калибровки порогов на конкретном двигателе (shadow-валидация обязательна)
- Carlucci 2006 — единственный opensource-источник числовых значений; при отсутствии полного PDF вендоров конкретные пороги амплитуд не используются

**Правила:** order_05_misfire_diesel (roadmap shadow → production), косвенно через 1.2 и 6.5

**Источники:**
- Antoni J. et al. (2004). Cyclostationary modelling of rotating machine vibration signals. *Mechanical Systems and Signal Processing*, 18(6):1285–1314. DOI:10.1016/S0888-3270(03)00088-8
- Carlucci A.P., Chiara F.F., Laforgia D. (2006). Vibration-based combustion monitoring in a common-rail diesel engine. *Journal of Sound and Vibration*, 295(1–2):141–164. DOI:10.1016/j.jsv.2005.12.054
- Gardner W.A. (1986). *Introduction to Random Processes with Applications to Signals and Systems*. Macmillan (основополагающая работа по циклостационарным процессам).

---

### A.29 Wavelet Analysis (CWT/DWT/WPT)

Вейвлет-анализ — частотно-временное представление сигнала с адаптивным разрешением: высокое временное разрешение на высоких частотах и высокое частотное разрешение на низких. Это отличает вейвлет от STFT, где разрешение одинаковое во всех полосах. Для диагностики подшипников и ударных событий вейвлет подходит лучше STFT: удар имеет локализованную форму (миллисекунды на килогерцовых частотах), и материнский вейвлет Morlet максимально коррелирует с формой отклика. Теоретические основы заложены Grossmann & Morlet (1984), Daubechies (1988, ортогональные базисы), Mallat (1989, пирамидальный алгоритм DWT). В автомобильной диагностике вейвлет применяется для дeтекции дисбаланса (Peng & Chu 2004), износа амортизаторов (Borowiec 2022, [7]), пропусков зажигания (wavelet packet transform). LLCAR использует вейвлет как будущее расширение envelope analysis: Morlet-CWT вместо bandpass filter + Hilbert.
...
Условие допустимости (admissibility):
  C_ψ = ∫ |Ψ(ω)|²/|ω| dω  < ∞
    Ψ(ω) = FFT{ψ(t)}
  ⟺  ψ имеет нулевое среднее: ∫ψ(t)dt = 0

Обратное преобразование:
  x(t) = (1/C_ψ) · ∫∫ W_x(a,b) · ψ((t−b)/a) · (1/a²) da·db

Discrete Wavelet Transform (DWT) — алгоритм Mallat (1989):
  a = 2^j,  b = k·2^j  (диадическая сетка)
  Реализация через QMF (Quadrature Mirror Filter) — пара low-pass h и high-pass g:
    cA_j[n]  = Σ h[2n−k] · cA_{j−1}[k]   (approximation, low-freq)
    cD_j[n]  = Σ g[2n−k] · cA_{j−1}[k]   (detail, high-freq)
  Пирамидальная структура: на уровне j сигнал поделен на cA_j (низкочастотная часть
  ниже fs/2^{j+1}) и cD_j (полоса [fs/2^{j+1}, fs/2^j]).

Wavelet Packet Transform (WPT):
  Дополняет DWT: high-pass каналы также рекурсивно разлагаются.
  Результат — равномерная частотная сетка (2^J полос одинаковой ширины Δf=fs/2^{J+1}).
  Применение: равноширокополосная фильтрация перед envelope analysis → близко к Kurtogram.

Материнские вейвлеты (выбор для LLCAR):
  Morlet (комплексный):   ψ(t) = π^(−1/4)·exp(jω₀t)·exp(−t²/2), ω₀≥5
    → Максимальная корреляция с откликом подшипникового удара (затухающая синусоида)
  Daubechies db4 – db10:  ортогональный, компактный носитель, разное число нулевых моментов
  Symlet sym4 – sym8:     почти симметричный вариант Daubechies
  Mexican Hat (Ricker):    ψ(t) = (1−t²)·exp(−t²/2), для детекции перегибов
  Meyer:                    гладкий в частотной области, для анализа сглаженных переходов
```

**Численные примеры — разложение DWT аудио-сигнала 44.1 кГц, 5 уровней:**

```
Уровень j    Полоса cD_j [Гц]         Типичное содержимое
1            11025–22050              Высокочастотный шум, cooling fan
2            5512–11025               Детонация, shuffle brake pads
3            2756–5512                Структурные резонансы головки блока
4            1378–2756                Ступичный подшипник, резонанс блока
5            689–1378                 Валы, CV-joint
cA_5         0–689                    Engine order 1–4, road hum

Для Morlet-CWT на fs=44.1 кГц с центральной частотой f_c=5 кГц:
  a = fs/(2πf_c·FWHM_ω) ≈ 1.4 при ω₀=5
  временное разрешение Δt ≈ 0.3 мс на 5 кГц
  частотное разрешение Δf/f ≈ 0.2 (Q-factor 5)
```

**Применение в LLCAR:**
Полный CWT не выполняется в реальном времени (требует FFT на каждом масштабе). Роadmap (Stage II) включает: (1) WPT-фильтрация аудио-фрейма → равноширокие полосы; (2) kurtosis каждого cD_j → выбор информативной полосы (аналог Kurtogram через WPT); (3) Hilbert envelope выбранной полосы → envelope FFT → BPFO detection. Этот pipeline описан Randall & Antoni (2011) как расширение envelope analysis. В настоящей версии LLCAR вейвлет используется косвенно — через ML-модели (Borowiec 2022), обученные на wavelet-признаках.

**Ограничения:**
- CWT требует O(N·M) операций, где M — число масштабов; 500 мс аудио на 44.1 кГц (N=22050) с M=64 → 1.4·10⁶ операций на окно → приемлемо для cloud, тяжело для edge-устройства
- Выбор материнского вейвлета — эвристический (Morlet для ударов, db4 для трендов)
- DWT не инвариантна к сдвигу (shift-variance); для диагностики ударных событий это критично → WPT + undecimated wavelet transform

**Правила:** roadmap для 1.3 (wheel_bearing_bpfo_harmonic, Stage II pipeline), косвенно [7] (Borowiec 2022)
...
- Mallat S. (1989). A theory for multiresolution signal decomposition. *IEEE Trans. PAMI*, 11(7):674–693. DOI:10.1109/34.192463
...
- Peng Z.K., Chu F.L. (2004). Application of the wavelet transform in machine condition monitoring. *Mechanical Systems and Signal Processing*, 18(2):199–221. DOI:10.1016/S0888-3270(03)00075-X
- Borowiec M. et al. (2022). A comprehensive diagnostic system for vehicle suspensions based on a neural classifier and wavelet resonance estimators. *Measurement*, 200:111602. DOI:10.1016/j.measurement.2022.111602

---

### A.30 Full Bearing Diagnostic Pipeline (Randall & Antoni 2011)

Randall & Antoni (2011) опубликовали авторитетный туториал, объединяющий все современные техники диагностики подшипников качения в единый pipeline: **SANC → Spectral Kurtosis → Kurtogram → Bandpass / WPT → Hilbert envelope → FFT**. Каждый шаг решает конкретную проблему: (1) SANC (Self-Adaptive Noise Cancellation) отделяет детерминированные компоненты (зубчатые колёса, порядки ДВС) от случайно-импульсного подшипникового сигнала; (2) SK + Kurtogram находят оптимальную полосу резонанса с максимальной импульсностью; (3) bandpass/WPT изолирует эту полосу; (4) Hilbert envelope извлекает модулирующий сигнал; (5) FFT показывает BPFO/BPFI как пики. Метод позволяет диагностировать Stage I дефект подшипника при SNR до −10 дБ. В LLCAR полный pipeline — roadmap Stage II: текущая версия использует только упрощённый BPFO-matching в 10 FFT-пиках (A.8), а Spectral Kurtosis шагает через shadow-правило 6.4.

**Математическая основа — численный пример ступичного подшипника 6206:**

```
Геометрия (SKF каталог):
  Z = 9 (число шариков)
  d = 9.5 мм (диаметр шарика)
  Dp = 46 мм (делительный диаметр)
  α = 0° (радиальный, cos α = 1)

Частоты дефектов (формулы A.8) при f_r = 10 Гц (колесо на 73 км/ч, D=0.65 м):
  BPFO = (Z/2) · f_r · (1 − d/Dp · cos α)
       = (9/2) · 10 · (1 − 9.5/46)
       = 4.5 · 10 · 0.7935
       = 35.71 Гц   ← ключевой пик наружного кольца

  BPFI = (Z/2) · f_r · (1 + d/Dp · cos α)
       = 4.5 · 10 · (1 + 0.2065)
       = 4.5 · 10 · 1.2065
       = 54.29 Гц   ← внутреннее кольцо

  FTF  = (f_r/2) · (1 − d/Dp · cos α)
       = 5.0 · 0.7935
       = 3.97 Гц    ← сепаратор (cage)

  BSF  = (Dp/(2·d)) · f_r · (1 − (d/Dp · cos α)²)
       = (46/(2·9.5)) · 10 · (1 − 0.2065²)
       = 2.42 · 10 · 0.9574
       = 23.18 Гц   ← дефект шарика (ball spin frequency)

Проверка ΣZ-правила: BPFO + BPFI = Z·f_r = 9·10 = 90.00 Гц ✓
                     35.71 + 54.29 = 90.00 ✓

Full pipeline:
  Шаг 1 (SANC): x_random = x − x_deterministic  (через adaptive LMS filter)
    Удаляет RPM-гармоники ДВС (order 1, 2, 3, 4) оставляя bearing + noise.

  Шаг 2 (Fast Kurtogram, A.26): kurtogram(x_random) → best_band = [3375, 3750] Гц
    Этот резонанс — первый структурный изгибный мод ступичного узла (bore-зависимый).

  Шаг 3 (bandpass): y = bandpass(x_random, 3375, 3750)

  Шаг 4 (Hilbert envelope, A.9): env = |hilbert(y)|

  Шаг 5 (envelope FFT): E(f) = FFT(env)
    Ожидается доминирующий пик на BPFO = 35.71 Гц
    Гармоники 2·BPFO = 71.4 Гц, 3·BPFO = 107.1 Гц
    Sidebands ±f_r = ±10 Гц (модуляция ротацией)
```

**Применение в LLCAR:**
Текущая версия LLCAR реализует только шаг 5 (FFT аудио) + matching с расчётным BPFO (правило 1.3). Полный pipeline требует raw audio ≥ 20 кГц и cloud-reprocessing; планируется Stage II: при `bpfo_harmonic_matches ≥ 2` триггерится cloud endpoint, выполняющий SANC + Kurtogram + Hilbert → точная стадия дефекта (I / II / III). Shadow-правило 6.4 (spectral_kurtosis_impulsive_bearing) — первая ступень этого pipeline: `spectral_kurtosis_audio > 3` в комбинации с `bpfo_harmonic_matches ≥ 1` даёт кандидата на cloud-reprocessing.

**Ограничения:**
- SANC-шаг требует синхронный reference-сигнал (RPM или порядки) — доступно через OBD; без него SANC вырождается в простой LMS denoiser
- Полный pipeline — высокочастотный O(N log N · M) и неприменим в реальном времени на смартфоне
- При отсутствии структурного резонанса в аудиополосе (< 16 кГц) bearing fault не детектируется даже полным pipeline

**Правила:** 1.3 (wheel_bearing_bpfo_harmonic), roadmap Stage II cloud-endpoint; 6.4 (shadow SK gate)

**Источники:**
- Randall R.B., Antoni J. (2011). Rolling element bearing diagnostics — A tutorial. *Mechanical Systems and Signal Processing*, 25(2):485–520. DOI:10.1016/j.ymssp.2010.07.017
- Antoni J. (2006). Spectral Kurtosis. *MSSP*, 20(2):282–307. DOI:10.1016/j.ymssp.2004.09.001
- Antoni J. (2007). Fast Kurtogram. *MSSP*, 21(1):108–124. DOI:10.1016/j.ymssp.2005.12.002
- Ho D., Randall R.B. (2000). Optimisation of bearing diagnostic techniques using SANC. *MSSP*, 14(5):763–788. DOI:10.1006/mssp.2000.1304
- Harris T.A., Kotzalas M.N. (2007). *Rolling Bearing Analysis*. 5th ed. CRC Press. ISBN 978-0-8493-7183-7

---

### A.31 Engine Order Spectrum и структурные резонансы

Двигатель внутреннего сгорания — многочастотный вибрационный источник с чётко предсказуемой сигнатурой. Каждая конструкция (L4, L6, V6, V8) имеет характерное распределение энергии по порядкам коленвала: частота вспышек f_fire = N_cyl · f_c / 2 (для 4-тактного), инерционные силы первого (F₁) и второго (F₂) рода, полгармонические резонансы ГРМ. Heywood (2018) даёт энциклопедическое описание источников; Crocker (2007) систематизирует структурные резонансы блока (800–1800 Гц), головки (1500–3500 Гц), поддона (400–900 Гц), впускного/выпускного тракта (200–1200 Гц). Отдельный сложный подклас — **детонация**: Draper (1938) вывел теоретическую частоту акустических мод в камере сгорания как собственные частоты колебаний газа в цилиндрической полости. Формула до сих пор применяется в заводских knock-сенсорах Bosch / Denso / Continental. Balансирные валы Lanchester (1904, английский патент 875899) компенсируют F₂ в L4-двигателях на удвоенной частоте коленвала.

**Математическая основа:**

```
Частота вспышек 4-тактного ДВС:
  f_fire = N_cyl · f_c / 2        [Гц]
    N_cyl = число цилиндров
    f_c   = обороты коленвала [об/с] = RPM/60

  Таблица f_fire при RPM=3000 (f_c=50 Гц):
    L3:  f_fire = 3·50/2  = 75 Гц   (order O=1.5)
    L4:  f_fire = 4·50/2  = 100 Гц  (O=2)
    L5:  f_fire = 5·50/2  = 125 Гц  (O=2.5)
    L6:  f_fire = 6·50/2  = 150 Гц  (O=3)
    V6:  f_fire = 6·50/2  = 150 Гц  (O=3, but banks offset)
    V8:  f_fire = 8·50/2  = 200 Гц  (O=4)
    V10: f_fire = 10·50/2 = 250 Гц  (O=5)
    V12: f_fire = 12·50/2 = 300 Гц  (O=6)

Инерционные силы (поршень + шатун, одноцилиндровый эквивалент):
  F₁(t) = m·r·ω²·cos(ωt)                    [1-й порядок]
  F₂(t) = m·r·ω²·(r/L)·cos(2ωt)             [2-й порядок, λ=r/L ≈ 0.25]
    m = приведённая масса ВКМ (0.5 кг типично)
    r = радиус кривошипа [м]
    ω = 2π·f_c

Балансирные валы Lanchester (для L4 без натуральной балансировки F₂):
  Два вала, вращающихся с угловой скоростью 2ω в противофазе → суммарная
  центробежная сила F_balance(t) = −F₂(t) → компенсация.
  Частота балансирного вала = 2·f_c = order 2 → в спектре на O=2.
  Отказ балансирного вала → аномально высокий амплитудный уровень
  на O=2 при неизменном O=1 (baseline) → новое правило order_2x_imbalance_l4.

Турбокомпрессор:
  n_turbo = 50 000 – 220 000 об/мин  (обычно 100–150 тыс. об/мин в крейсе)
  f_turbo = n_turbo / 60             [Гц]
  f_blade = z · f_turbo              [Гц]
    z = число лопаток турбины/компрессора, обычно 10–14
  При n_turbo=150000 об/мин, z=12: f_blade = 2500·12 = 30 000 Гц
  ← выше nyquist смартфонного аудио (22 кГц) → не детектируется
  При n_turbo=80000 об/мин, z=12: f_blade = 16 000 Гц — в пределах аудиополосы

Частота детонации Draper (1938):
  f_{m,n} = ρ_{m,n} · c / (π · B)       [Гц]
    ρ_{m,n} = корни уравнения J'_m(ρ) = 0 (азимутальные моды)
               для (1,0) моды: ρ = 1.841  (первая окружная)
               для (2,0) моды: ρ = 3.054  (вторая окружная)
               для (0,1) моды: ρ = 3.832  (первая радиальная)
    c  = скорость звука в газах сгорания ≈ 900–1000 м/с при T=2500 K
    B  = диаметр цилиндра (bore) [м]

  ВАЖНО (терминология): ρ = 1.841 — корень J'_1(ρ)=0, соответствует
  ПЕРВОЙ ОКРУЖНОЙ (1,0) моде (circumferential), а НЕ радиальной.
  В старой литературе встречается неточность «radial» — корректный
  термин по Eisen (1974) и Brecq (2003) = «circumferential (1,0)».
  Первая чисто радиальная мода (0,1) даёт ρ=3.832 → f ≈ 2× выше.

Источник Draper: Draper C.S. (1938). The physical effects of detonation
in a closed cylindrical chamber. NACA Technical Report. DOI: 10.2514/8.590

ВАЖНО (год): в некоторых источниках ошибочно указывается 1933 —
...
Полоса knock-сенсора Bosch (ME7.x, MED17):  5–9 кГц типично — охватывает B=72–100 мм

Структурные резонансы (Crocker 2007):
  Блок цилиндров (cast iron, aluminium):       800–1800 Гц
  Головка блока:                               1500–3500 Гц
  Поддон картера (oil pan):                    400–900 Гц
  Впускной коллектор (acoustic + structural): 200–1200 Гц
  Выпускной коллектор + катализатор:          300–1000 Гц + 2×RPM резонанс
  Крышка клапанов:                             1200–2800 Гц

Полгармонические порядки (O=0.5 и гармоники):
  Ремень/цепь ГРМ:               O=0.5 (полоборота за 1 оборот коленвала)
  Распредвал (single):           O=0.5
  Misfire (пропуск в одном циле): O=0.5 (1 пропуск за 2 оборота)
  Уравновешивающий вал (Subaru EJ25): O=0.5

Carlucci (2006) — дизель: O=0.5 качественно возрастает при пропусках,
конкретные амплитуды только на опорных данных (без полного PDF не
используются в LLCAR — см. ограничения A.28).
```

**Применение в LLCAR:**
- Правило 1.15 (knock_impulse_percussive): текущая жёсткая полоса 5–8 кГц заменяется динамической `knock_expected_freq_from_bore` (VIN → bore → Draper). Если bore известен, правило использует полосу f_{1,0} ± 1.5 кГц; иначе fallback 5–8 кГц.
- Правило order_2x_imbalance_l4 (roadmap): `cylinder_count == 4` + `order_2x_amp z>2.5` при нормальном order_1x_amp → отказ балансирных валов Lanchester.
- Правило 1.2 (engine_mount_harmonic): дополнительно проверка попадания в f_fire = N_cyl·RPM/120 через `rpm_order_matches`.
- Turbo blade detection (roadmap Session 5): при наличии `turbo_blade_count` в VIN-профиле рассчитывается ожидаемая f_blade; в LLCAR доступно только при n_turbo < 100 000 об/мин (f_blade < 20 кГц).

**Ограничения:**
- Знание bore требует расширения VIN-декодера; для неизвестного bore fallback на стандартную полосу 5–9 кГц
- Draper формула справедлива для идеального цилиндра; реальная камера имеет сложную геометрию → наблюдаемая частота детонации может отклоняться на ±15% от f_{1,0}
- Частота турбины выходит за аудиополосу смартфона при n_turbo > 100 тыс. об/мин
- Балансирные валы Lanchester не присутствуют на всех L4 (например, L4 < 2.0 L часто без балансирных валов) — правило требует базы данных по моделям

**Правила:** 1.15 (обновление knock полосы через Draper), 1.2 (order verification), roadmap order_2x_imbalance_l4, roadmap turbo_blade_pass

**Источники:**
- Draper C.S. (1938). Physical effects of detonation in a closed cylindrical chamber. *NACA Technical Report* No. 493. DOI:10.2514/8.590
- Brecq G. et al. (2003). A new indicator for knock detection in gas SI engines. *SAE Technical Paper* 2003-01-1915. DOI:10.4271/2003-01-1915
- Di Gaeta A. et al. (2018). A new physics-based model of engine knock for advanced spark-timing control. *Applied Energy*, 221:1–11. DOI:10.1016/j.apenergy.2018.03.100
- Heywood J.B. (2018). *Internal Combustion Engine Fundamentals*. 2nd ed. McGraw-Hill. ISBN 978-1-260-11610-6
- Crocker M.J. (ed.) (2007). *Handbook of Noise and Vibration Control*. Wiley. ISBN 978-0-471-39599-7
- Lanchester F.W. (1904). British Patent GB 875899 — Balance of reciprocating engines.

---

### A.32 BOGE Forced Vibration (MAHA FWT)

BOGE-тест (также известный как MAHA FWT, Fahrwerktester) — метод стендовой диагностики амортизаторов, основанный на измерении максимальной амплитуды вынужденных колебаний кузова A_max при фиксированной частоте возбуждения вблизи резонанса неподрессоренной массы (≈ 8–10 Гц). В отличие от EUSAMA (A.20), измеряющего минимальную нормальную реакцию F_min, BOGE измеряет пиковую амплитуду вертикального перемещения. Стенд выполняет sweep по частоте в районе резонанса колеса (wheel hop), регистрирует A_max, и сравнивает с эталонными значениями от производителя стенда (MAHA, Hofmann, Boge). Klapka et al. (2017) в обзорной статье «Twilight of EUSAMA» показали, что BOGE/MAHA FWT менее чувствителен к давлению шин, чем EUSAMA, но имеет собственные ограничения: сильная зависимость от температуры амортизатора и трения колеса о плиту.

**Математическая основа:**

```
Принцип BOGE (forced vibration resonance):
  Стенд → синусоидальное возбуждение z_stand(t) = A_stand · sin(2π·f·t)
  Ответ кузова → z_body(t) = A_body(f) · sin(2π·f·t − φ(f))

  Transmissibility T(f) = A_body(f) / A_stand(f)

  Для quarter-car модели (A.3):
    T(f) =  √((1 + 4ζ²·(f/f_n)²) / ((1−(f/f_n)²)² + 4ζ²·(f/f_n)²))

  Максимум T при f ≈ f_n·√(1−2ζ²) (damped resonance)
  Wheel hop частота f_n ≈ 10–12 Гц (для большинства легковых авто)

Классификация MAHA FWT (vendor specification):
  A_max [мм]        Состояние             Рекомендация
  0 – 25            Good (новый)          No action
  25 – 50           Deteriorated          Monitor / adjust
  > 50              Unacceptable          Replace

Связь с damping coefficient:
  Большой A_max ⟺  малое ζ ⟺  малое C (damping)
  A_max / A_stand максимально при ζ → 0 → при разрушенном амортизаторе

ВАЖНО (атрибуция): пороги 0–25 / 25–50 / >50 мм — это MAHA vendor
consensus, а не международный ISO/SAE стандарт. При адаптации к другим
стендам (Hofmann Brekon, Boge SchwingGold) уровни могут отличаться ±20%.
```

**Численные примеры — BOGE vs EUSAMA для одного и того же амортизатора:**

```
Состояние              EUSAMA W_E [%]   BOGE A_max [мм]   Примечание
Новый амортизатор      ≥ 50             15–20             Оба метода согласны
Износ 30%              40–50            25–35             BOGE раньше реагирует
Выработан (утечка)     15–25            55–70             Оба: плохо
Парадокс «сухое трение» 40+              60+              EUSAMA пропускает!

Парадокс сухого трения — критическое ограничение EUSAMA:
Заклинивший поршень (коксованное масло, ржавчина) — F_min остаётся высоким
(колесо НЕ отрывается), W_E показывает «хорошо», но A_body резко падает
при BOGE-тесте (колебания «заглушены»). BOGE fangs заклинивание лучше.
```

**Применение в LLCAR:**
Новое shadow-правило 6.7 (damping_bandwidth_wide_shadow) косвенно реализует BOGE-логику через proxy: пик amplitude spectrum акселерометра в полосе wheel hop [9, 14] Гц + широкий пик (hpbm_bandwidth_ratio > 0.5) → низкое damping → подозрение на износ. Полный BOGE недоступен на смартфоне (требует стенда); для моделей с OBD (жёсткая платформа) можно инжектировать stand_boge_result через манyal entry (stand-import правило 1.30 roadmap).

**Ограничения:**
- BOGE требует стенд — невозможно реплицировать на смартфоне
- Пороги A_max vendor-specific, не ISO-стандартизированы
- Сильная зависимость от температуры: при T < 0°C амортизатор «густее» → A_max падает → ложно-«хороший» результат
- Не различает передний/задний или левый/правый амортизаторы при одновременной тестировании колёс

**Правила:** 6.7 (shadow proxy), roadmap 1.30 (BOGE manual entry при ТО)

**Источники:**
- Klapka M. et al. (2017). Twilight of the EUSAMA diagnostic methodology. *Meccanica*, 52(1–2):265–278. DOI:10.1007/s11012-016-0566-0
- Konieczny L., Filipczyk J. (2022). Technical condition diagnostics of vehicle shock absorbers using MAHA FWT-2 test bench. *Diagnostyka*, 23(4):2022415. DOI:10.29354/diag/156917
- MAHA Maschinenbau Haldenwang (2019). FWT Fahrwerktester Product Specification. Vendor docs.
- Directive 2014/45/EU. Periodic roadworthiness tests for motor vehicles and their trailers. OJ L 127, 29.4.2014, p. 51.

---

### A.33 Phase Angle Method (Tsymberov)

Phase Angle Method — альтернатива EUSAMA, измеряющая не амплитуду, а **фазовый сдвиг** φ между возбуждением стенда и ответом кузова. Метод предложен Anatol Tsymberov (SAE 960735, 1996) и стандартизирован в ряде национальных протоколов. Физика: при резонансе (f ≈ f_n) фазовый сдвиг между z_stand и z_body равен ровно 90° для любого демпфирования; ниже резонанса φ < 90°; выше — φ > 90°; при очень сильном износе φ достигает 180° (противофаза). Метрика U_min = 180° − φ_max (дополнение максимального фазового сдвига до 180°) напрямую обратно пропорциональна качеству демпфирования: большой U_min → хороший амортизатор; U_min близок к 0° → полный отказ. Главное преимущество Phase Angle над EUSAMA — **слабая чувствительность к давлению шины и massовым параметрам автомобиля**, что делает метод более надёжным в полевых условиях (Guzek 2021 подтверждает: изменение давления шин 1.6 → 3.0 бар меняет EUSAMA % на ±15%, Phase Angle U_min — на ±3%).

**Математическая основа:**

```
Фазовый сдвиг в quarter-car модели (A.3):
  Возбуждение: z_stand(t) = A·sin(ωt)
  Ответ кузова: z_body(t) = B·sin(ωt − φ)

  φ(ω) = arctan(2ζ·(ω/ω_n) / (1 − (ω/ω_n)²))

  При ω = ω_n (резонанс):      φ = 90°  (π/2)
  При ω << ω_n:                 φ → 0°   (кузов следует за стендом)
  При ω >> ω_n:                 φ → 180° (кузов "стоит", стенд "бегает")

  При сильном демпфировании (ζ → 1):
    фазовая характеристика φ(ω) пологая, max наклон мал
  При слабом демпфировании (ζ → 0):
    φ(ω) переключается с 0° на 180° скачком на f_n → max наклон велик

Метрика Tsymberov U_min:
  U_min = 180° − φ_max         [°]
  где φ_max = максимум фазового сдвига при sweep-е около f_n

Пороги Tsymberov (SAE 960735):
  U_min [°]      Состояние амортизатора
  > 60           Good (new)
  40 – 60        Acceptable
  20 – 40        Deteriorated
  < 20           Bad (replace)

ВАЖНАЯ АТРИБУЦИЯ (исправление из первоначальных источников):
Пороги U_min — это дословно из Tsymberov A., SAE 960735 (1996). В некоторых
более поздних публикациях (включая Konieczny 2022) встречаются те же числа,
но Konieczny ссылается на Tsymberov, а не формулирует собственные пороги.
Первоисточник — Tsymberov 1996, DOI:10.4271/960735.

Guzek 2021 (DOI:10.26552/com.C.2021.3.B178-B186) демонстрирует таблично:
  Изменение P_tire 1.6 → 3.0 бар  →  EUSAMA W_E: ±15% (чувствителен)
                                      U_min: ±3% (устойчив)
```

**Численные примеры — Phase Angle vs EUSAMA sensitivity:**

```
Состояние               EUSAMA W_E     φ_max      U_min    ζ eq.
Новый                   55%             160°       20°      0.32
Минимальный износ       48%             155°       25°      0.28
Умеренный износ         38%             150°       30°      0.22
Значительный износ      25%             135°       45°      0.15
Утечка (dead)           12%             100°       80°      0.05

Влияние давления шин (Guzek 2021):
  P=1.6 бар, новый:  EUSAMA 42%, U_min 22°
  P=2.2 бар, новый:  EUSAMA 52%, U_min 21°   ← EUSAMA скачок +10%
  P=3.0 бар, новый:  EUSAMA 58%, U_min 20°   ← U_min почти не изменился
```

**Применение в LLCAR:**
Shadow-правило 6.6 (phase_lag_shift_shadow) — дорожный proxy для phase angle через кросс-корреляцию каналов `ax` (лонгитудинал) и `az` (вертикаль) в окне ±0.5 с после ударного события (яма, лежачий полицейский). Метрика `ax_az_phase_proxy` — коэффициент Пирсона между отклоняющимися z_body и z_wheel (смартфон не имеет отдельного wheel-акселерометра, но при достаточной жёсткости кузова ax и az коррелируют с разной фазой). При `|ax_az_phase_proxy| < 0.3` (слабая синхронизация) + `az_std z>1.5` + `crest_factor_z > 3.5` → подозрение на плохое демпфирование. Полный Phase Angle доступен только на стенде (roadmap 1.31 manual entry).

**Ограничения:**
- Смартфон не различает z_wheel и z_body → ax_az_phase_proxy работает через статистический proxy, не через истинный фазовый сдвиг
- Нужна строгая скоростная нормировка: при speed < 40 км/ч proxy зашумлён
- Полный Phase Angle требует 2 синхронных датчика на кузове и колесе → стенд или инструментированный автомобиль
- При «сухом трении» амортизатора (заклинивший поршень) Phase Angle может дать ложно-«хороший» U_min — BOGE чувствительнее

**Правила:** 6.6 (shadow proxy), roadmap 1.31 (Phase Angle manual entry)

**Источники:**
- Tsymberov A. (1996). An improved non-intrusive automotive suspension testing apparatus with means to determine the condition of the dampers. *SAE Technical Paper* 960735. DOI:10.4271/960735
- Konieczny L., Filipczyk J. (2022). Technical condition diagnostics of shock absorbers. *Diagnostyka*, 23(4):2022415. DOI:10.29354/diag/156917
- Zdanowicz P., Guzek M. (2021). Diagnostics of On-Vehicle Shock Absorber Testing. *Communications*, 23(3):B178–B186. DOI:10.26552/com.C.2021.3.B178-B186
- Klapka M. et al. (2017). Twilight of the EUSAMA diagnostic methodology. *Meccanica*, 52:265–278. DOI:10.1007/s11012-016-0566-0

---

### A.34 Half-Power Bandwidth Method (Calvo)

Half-Power Bandwidth Method (HPBM) — классический метод определения коэффициента демпфирования ζ из ширины резонансного пика на уровне −3 дБ. Метод применяется в механике со времён Thomson & Tait (1879) и адаптирован Calvo et al. (2005) для диагностики автомобильной подвески. Физика: при вынужденных колебаниях transmissibility T(ω) имеет пик на ω_n шириной Δω = ω₂ − ω₁, измеряемой на уровне T_max/√2 (−3 дБ). Связь: ζ = Δω / (2·ω_n). Для новых амортизаторов ζ = 0.25–0.40, пик узкий; для изношенных ζ → 0, пик узкий и высокий (но амплитуда выходит на нелинейный уровень); для мёртвых амортизаторов ζ очень мало, но пик сильно искажён из-за сухого трения → HPBM неприменим. Guard-условие: HPBM корректно работает только при ζ < 0.25 (явный резонанс); за пределами нужен SDOF curve fitting.

**Математическая основа:**

```
Transmissibility SDOF (A.3):
  T(ω) = |Z_body(ω) / Z_stand(ω)|
       = √((1 + 4ζ²η²) / ((1 − η²)² + 4ζ²η²))
  где η = ω / ω_n

Максимум T на ω = ω_r = ω_n · √(1 − 2ζ²)
  T_max ≈ 1 / (2ζ·√(1−ζ²))  ≈ 1/(2ζ) при малом ζ

HPBM — ширина на уровне T_max/√2 (−3 дБ):
  Δω = ω₂ − ω₁  (где T(ω₁) = T(ω₂) = T_max/√2)
  ζ = Δω / (2·ω_n)                         [безразмерный]

  Эквивалентно в частотах:
  ζ = Δf / (2·f_n) = (f₂ − f₁) / (2·f_n)

Пороги (Calvo 2005):
  ζ > 0.40     Overdamped (редко — sport/stiff setup)
  0.25 – 0.40  Good (OEM normal range)
  0.15 – 0.25  Acceptable
  0.08 – 0.15  Deteriorated
  < 0.08       Bad (replace)

Guard-условие применимости:
  HPBM корректно при T_max > 2 (явно выраженный резонанс)
  Эквивалентно: ζ < 0.35, иначе пик «размазан» и HPBM даёт ошибку
  hpbm_applicable = true  ⟺  max(T) > 2 AND симметрия пика |ω_r − (ω₁+ω₂)/2|/ω_r < 0.1

Связь с логарифмическим декрементом (A.21):
  δ = 2π·ζ / √(1 − ζ²)  ≈ 2π·ζ  при малом ζ

Hryciów et al. (2021) — расширение HPBM для асимметричных пиков (нелинейное demping):
  Использует левую и правую половины пика отдельно → ζ_left, ζ_right.
  Разница |ζ_left − ζ_right| > 0.05 → асимметрия = нелинейность = износ.
```

**Численные примеры — HPBM на quarter-car с f_n = 12 Гц:**

```
ζ       f₁ [Гц]   f₂ [Гц]   Δf [Гц]   T_max    Δf/f_n   Состояние
0.40    9.6       14.4      4.8       1.31     0.40     Overdamped
0.25    10.8      13.2      2.4       2.07     0.20     Good
0.15    11.3      12.7      1.4       3.37     0.117    Acceptable
0.08    11.68     12.32     0.64      6.27     0.053    Deteriorated → зона замены

Пример — реальный замер на дорожном полигоне:
  wheel_hop_peak_freq = 11.8 Гц
  half-power left:  f₁ = 10.7 Гц
  half-power right: f₂ = 12.9 Гц
  Δf = 2.2 Гц
  hpbm_bandwidth_ratio = Δf / f_n = 2.2/11.8 = 0.186
  ζ = 0.186/2 = 0.093  → Deteriorated
```

**Применение в LLCAR:**
Shadow-правило 6.7 (damping_bandwidth_wide_shadow) использует `hpbm_bandwidth_ratio` = ширина пика wheel hop (z-акселерометр FFT) делённая на пиковую частоту. Guard-флаг `hpbm_applicable` = true только когда T_max > 2 и симметрия пика < 10%. Пороги: `hpbm_bandwidth_ratio > 0.50` → widely-damped (суррогат ζ < 0.12) → деградация. Комбинация с wheel_hop_peak_freq [9, 14] Гц фильтрует не-резонансные FFT-пики. В отличие от EUSAMA (1.6), HPBM работает при любой скорости > 40 км/ч и не требует стенда; в отличие от Phase Angle (6.6), HPBM устойчив к заклиниванию амортизатора (симметрия пика нарушается при сухом трении → hpbm_applicable=false → правило не срабатывает → false positive исключён).

**Ограничения:**
- HPBM неприменим при сильном нелинейном демпфировании (heavy dry friction, ζ > 0.35)
- Требует чёткого пика — в городском цикле (переменная скорость, короткие окна) часто не выполняется
- Разрешение FFT 500 мс = 2 Гц, но типичная Δf на f_n = 12 Гц при хорошем амортизаторе = 2–3 Гц → недостаточное разрешение → нужны окна ≥ 1.5 с
- Не различает передний/задний; для поколесной диагностики нужны wheel-акселерометры

**Правила:** 6.7 (shadow proxy), roadmap 1.32 (HPBM manual entry при ТО)

**Источники:**
- Calvo J.A. et al. (2005). An experimental method to study the dynamic response of shock absorbers. *Int. Journal of Vehicle Design*, 38(2/3):172–188. DOI:10.1504/IJVD.2005.007623
...
[52] Konieczny L., Filipczyk J. (2022). Technical condition diagnostics of vehicle
     shock absorbers using MAHA FWT-2 test bench. Diagnostyka, 23(4):2022415.
     DOI: 10.29354/diag/156917
     Используется в: A.32 (BOGE/MAHA FWT), обзор альтернатив EUSAMA.

[53] Klapka M. et al. (2017). Twilight of the EUSAMA diagnostic methodology.
     Meccanica, 52(1–2):265–278.
     DOI: 10.1007/s11012-016-0566-0
     Используется в: A.20 (EUSAMA ограничения), A.32 (BOGE сравнение),
     A.33 (Phase Angle vs EUSAMA).

[54] Calvo J.A. et al. (2005). An experimental method to study the dynamic response
     of shock absorbers. International Journal of Vehicle Design, 38(2/3):172–188.
     DOI: 10.1504/IJVD.2005.007623
     Используется в: A.34 (Half-Power Bandwidth Method, первоисточник порогов ζ).

[55] Tsymberov A. (1996). An improved non-intrusive automotive suspension testing
     apparatus with means to determine the condition of the dampers.
     SAE Technical Paper 960735.
     DOI: 10.4271/960735
     Используется в: A.33 (Phase Angle Method, первоисточник порогов U_min).
     ПРИМЕЧАНИЕ: пороги U_min — это Tsymberov 1996, а НЕ Konieczny 2022.

[56] Fyfe K.R., Munck E.D.S. (1997). Analysis of computed order tracking.
     Mechanical Systems and Signal Processing, 11(2):187–205.
     DOI: 10.1006/mssp.1996.0056
     Используется в: A.27 (Computed Order Tracking, первоисточник алгоритма).

[57] Antoni J. (2006). The spectral kurtosis: a useful tool for characterising
     non-stationary signals. Mechanical Systems and Signal Processing, 20(2):282–307.
     DOI: 10.1016/j.ymssp.2004.09.001
     Используется в: A.26 (SK определение), A.30 (full bearing pipeline).

[58] Antoni J. (2007). Fast computation of the kurtogram for the detection of
     transient faults. Mechanical Systems and Signal Processing, 21(1):108–124.
     DOI: 10.1016/j.ymssp.2005.12.002
     Используется в: A.26 (Fast Kurtogram алгоритм), правило 6.4 shadow.

[59] Randall R.B., Antoni J. (2011). Rolling element bearing diagnostics — A tutorial.
     Mechanical Systems and Signal Processing, 25(2):485–520.
     DOI: 10.1016/j.ymssp.2010.07.017
     Используется в: A.30 (full bearing pipeline — основная ссылка), A.26, A.27.

[60] Carlucci A.P., Chiara F.F., Laforgia D. (2006). Vibration-based combustion
     monitoring in a common-rail diesel engine. Journal of Sound and Vibration,
     295(1–2):141–164.
     DOI: 10.1016/j.jsv.2005.12.054
     Используется в: A.28 (cyclostationary, misfire O=0.5 — только качественно).

[61] Peng Z.K., Chu F.L. (2004). Application of the wavelet transform in machine
     ...
     Используется в: A.29 (wavelet обзор).

[62] Hryciów Z. et al. (2021). The influence of shock absorber wear on vehicle
     dynamic properties. Eksploatacja i Niezawodność, 23(2):274–282.
     DOI: 10.17531/ein.2021.2.14
     Используется в: A.34 (HPBM асимметрия пика при нелинейном demping).

[63] Draper C.S. (1938). Physical effects of detonation in a closed cylindrical
     chamber. NACA Technical Report No. 493.
     DOI: 10.2514/8.590
     Используется в: A.31 (формула частоты детонации, ρ=1.841).
     ПРИМЕЧАНИЕ: корректный год — 1938 (в некоторых источниках ошибочно 1933).

[64] Antoni J. et al. (2004). Cyclostationary modelling of rotating machine vibration
     signals. Mechanical Systems and Signal Processing, 18(6):1285–1314.
     DOI: 10.1016/S0888-3270(03)00088-8
     Используется в: A.27 (COT), A.28 (cyclostationary framework, первоисточник).

[65] Gobbi M. et al. (2008). Optimal design of complex mechanical systems with
     applications to vehicle engineering — quarter-car case. Meccanica, 43(6):649–664.
     DOI: 10.1007/s11012-008-9119-5
     Используется в: A.34 (HPBM валидация).

[66] Brecq G. et al. (2003). A new indicator for knock detection in gas SI engines.
     SAE Technical Paper 2003-01-1915.
     DOI: 10.4271/2003-01-1915
     Используется в: A.31 (верификация Draper формулы, (1,0) мода ρ=1.841).

[67] Di Gaeta A. et al. (2018). A new physics-based model of engine knock for advanced
     spark-timing control. Applied Energy, 221:1–11.
     DOI: 10.1016/j.apenergy.2018.03.100
     Используется в: A.31 (современное подтверждение Draper при переменной c(T)).

[68] Daubechies I. (1988). Orthonormal bases of compactly supported wavelets.
     Communications on Pure and Applied Mathematics, 41(7):909–996.
     DOI: 10.1002/cpa.3160410705
     Используется в: A.29 (DWT ортогональные базисы).

[69] Mallat S. (1989). A theory for multiresolution signal decomposition: the wavelet
     representation. IEEE Trans. PAMI, 11(7):674–693.
     DOI: 10.1109/34.192463
     Используется в: A.29 (пирамидальный DWT, QMF).

[70] Ho D., Randall R.B. (2000). Optimisation of bearing diagnostic techniques using
     simulated and actual bearing fault signals (SANC). Mechanical Systems and Signal
     Processing, 14(5):763–788.
     DOI: 10.1006/mssp.2000.1304
     Используется в: A.30 (SANC — шаг 1 full bearing pipeline).
```
...
[S16] ISO 20816-3:2022 — Mechanical vibration. Measurement and evaluation of machine
                          vibration. Part 3: Industrial machinery with nominal power
                          above 15 kW. (Заменяет ISO 10816-3:2009 [S4]).
                          Используется в: диагностика вращающихся узлов (подшипники,
                          роторы турбо) — обновлённая альтернатива S4.

[S17] ISO 21940-11:2016 — Mechanical vibration. Rotor balancing. Part 11: Procedures
                          and tolerances for rotors with rigid behaviour.
                          (Заменяет ISO 1940-1:2003).
                          Используется в: A.27 Order Tracking (балансировочные пороги
                          на порядке O=1), будущее правило wheel_balance_OBD.

[S18] ISO 13373-3:2015 — Condition monitoring and diagnostics of machines. Vibration
                          condition monitoring. Part 3: Guidelines for vibration
                          diagnosis.
                          Используется в: A.9 Envelope Analysis, A.30 bearing pipeline.

[S19] ISO 15242-1:2015 — Rolling bearings. Measuring methods for vibration. Part 1:
                          Fundamentals.
                          Используется в: A.8 BPFO/BPFI, A.30 full bearing pipeline.

[S20] ISO 18431-1:2005 — Mechanical vibration and shock. Signal processing. Part 1:
                          General introduction.
                          Используется в: A.10 FFT, A.26 Spectral Kurtosis, A.29 Wavelet.

[S21] VDI 3832:2013    — Measurement of structure-borne sound of rolling element
                          bearings in machines and plants for evaluation of condition.
                          Используется в: A.9 Envelope Analysis, A.30 bearing pipeline
                          (немецкий аналог ISO 13373-3).

[S22] Directive 2014/45/EU — Periodic roadworthiness tests for motor vehicles and their
                          trailers. Official Journal L 127, 29.4.2014, p. 51.
                          Используется в: A.20 EUSAMA, A.32 BOGE/MAHA FWT, A.33 Phase
                          Angle — нормативная база для стендовых методов ТО в ЕС.

[S23] SAE J670:2008    — Vehicle Dynamics Terminology.
                          Используется в: A.2 Half-Car, A.3 Quarter-Car — стандарт
                          терминологии (wheel hop, heave, pitch, roll).

[S24] EUSAMA TS-02/76  — EUSAMA Technical Specification 02/76: Recommendations for the
                          testing of the suspension dampers on vehicles (первоисточник
                          1976 г., sweep 25→0 Hz, амплитуда 3 мм, W_E метрика).
                          Используется в: A.20 EUSAMA (оригинальная методология,
                          переиздан как EUSAMA TR 2000 [S11]).
                          ПРИМЕЧАНИЕ: конкретные пороги W_E ≥ 61% / 41% / 21% НЕ
                          присутствуют в TS-02/76 1976 г. — это vendor consensus
                          MAHA/Hofmann/Boge (подтверждено Guzek 2021).
```
