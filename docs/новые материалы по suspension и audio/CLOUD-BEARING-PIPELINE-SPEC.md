...
## О чём это простыми словами

**Задача:** поймать износ подшипника колеса на самой ранней стадии, когда его ещё не слышно ухом и стенд его ещё не видит.

**Как сейчас (Stage 0):** телефон в машине сам сжимает 60 сек звука и вибрации в ~1.5 КБ сводки (несколько цифр: «насколько ударно», «в какой полосе частот шумит сильнее всего»). Эти цифры улетают на сервер. Правило смотрит на них и говорит «похоже подшипник / непохоже». Работает на уже заметных дефектах, ранние пропускает.

**Что предлагает эта спека (Stage I):** раз в сутки по Wi-Fi телефон вместо сжатой сводки шлёт на сервер **сырой звук и вибрацию целиком** (~1.8 МБ в сжатом виде, Parquet). На сервере крутится отдельный воркер, который делает 6 шагов обработки — это и есть алгоритм Randall-Antoni 2011 (стандарт в диагностике подшипников):

1. **SANC** — вырезает шум от шестерён и двигателя, чтобы остался только «стук» подшипника.
2. **Spectral Kurtosis + Kurtogram** — автоматически находит, в какой полосе частот этот стук максимально виден (вместо того чтобы фиксировать «ищи в 3–5 кГц»).
...
6. **BPFO-детектор** — сравнивает найденную частоту с теоретической частотой прохождения тел качения по наружному кольцу (считается из геометрии подшипника). Если совпадает ± 1.5 Гц на 3+ гармониках — дефект есть.

**Зачем это отдельно от Stage 0:** на ранних дефектах сигнал маскируется дорожным шумом, и сжатые сводки его не видят. Полный pipeline по сырому сигналу ловит дефект на 7–14 дней раньше.

**Почему ниже 7 блоков:** это не 7 фич, это 7 мест в коде/инфре, которые надо написать:
1. Добавить в Android-приложение кнопку «разрешить загрузку сырого сигнала» + опцию Wi-Fi-only.
2. Новый endpoint на сервере `POST /api/v2/bearing-raw/` + таблица в PostgreSQL для очереди задач.
3. Worker-скрипт на сервере, который берёт задачу из очереди и прогоняет 6 шагов алгоритма.
4. Новое production-правило `wheel_bearing_bpfo_cloud_stage1`, которое читает результат.
5. Проверка на bench-подшипнике (реальный подшипник 6206 с искусственным дефектом — иначе алгоритм не валидирован).
6. Панель в дашборде `BearingStage1Panel.tsx` с графиком envelope-спектра.
7. Roadmap — как разбить на 5 рабочих сессий.

**Критические риски (почему это не делается за одну сессию):**
- **Storage-бюджет:** 1000 устройств × 1 файл/день × 1.8 МБ × 90 дней = 160 ГБ диска. На сервере сейчас 40 ГБ свободно. Нужно расширение.
- **GDPR/речь:** аудио из салона = записывается разговор. Нужен band-pass ≥ 500 Гц на телефоне ДО загрузки, чтобы речь вырезалась.
- **Polling-воркер:** MVP без Celery работает до ~10 тыс. задач/день. После этого будет деградировать — понадобится Celery/Redis или Postgres queue с blue-green.
- **Opt-in <5 %:** если пользователи не разрешат, датасет растёт медленно, валидация откладывается.

Ниже — полная техническая спецификация этих 7 блоков.
