...
export function Diagnostics() {
  ...
  // Panel expand state (Row 2 instruments only)
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 0: TRUST BAR — полная ширина, компактная полоса
          ═══════════════════════════════════════════════════════════════════ */}
      <GlassPanel>
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2">
            {expertMode && (
              <button
                onClick={toggleV2Api}
                className="nav-btn font-mono text-[10px] lg:text-xs px-2 lg:px-3 py-1 rounded transition-all duration-200"
                style={{
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 1: HERO — диагностический результат + статус систем
          ═══════════════════════════════════════════════════════════════════ */}
      <div className="grid grid-cols-12 gap-3">
        {/* Left: DiagnosisCard + Run button (col-span-8) */}
        ...
        {/* Right: System Checklist + Health Bars stacked (col-span-4) */}
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-3">
          {/* System Checklist (V2 only) */}
          {useV2Api && v2Report && (
            <GlassPanel>
              ...
              <div className="grid grid-cols-2 gap-2">
                {([
                  ...
                  const stLabel = st === 'ok' ? 'Норма' : st === 'warning' ? 'Внимание' : 'Проблема'
                  return (
                    <div key={sys.key} style={{
                      ...
                      <span style={{ fontSize: 18, color: stColor, textShadow: `0 0 8px ${stColor}40` }}>{checkIcon}</span>
                      <div>
                        <div style={{ fontSize: 11, fontFamily: "'Rajdhani', sans-serif", fontWeight: 700, color: theme.text.primary }}>
                          {sys.icon} {sys.name}
                        </div>
                        <div style={{ fontSize: 9, fontFamily: "'Rajdhani', sans-serif", color: stColor, fontWeight: 600 }}>
                          {stLabel} · {score}
                        </div>
                      </div>
                    </div>
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 2: ВИЗУАЛЬНЫЕ ИНСТРУМЕНТЫ — равные трети, клик для раскрытия
          ═══════════════════════════════════════════════════════════════════ */}
      <div className="grid grid-cols-12 gap-3">
        {/* SmartSphere 3D */}
        <div
        ...
        {/* AudioSpectrum */}
        <div
          className={`${expanded === 'audio' ? 'col-span-12' : expanded ? 'hidden' : 'col-span-12 md:col-span-6 lg:col-span-4'} cursor-pointer transition-all duration-300`}
          onClick={() => toggle('audio')}
        >
          <AudioSpectrum data={audioData} />
        </div>

        {/* AnomalyTimeline + TimelineScrubber */}
        <div
          className={`${expanded === 'timeline' ? 'col-span-12' : expanded ? 'hidden' : 'col-span-12 lg:col-span-4'} cursor-pointer transition-all duration-300`}
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 3: TRUST ENGINE — полная ширина
          ═══════════════════════════════════════════════════════════════════ */}
      <div className="grid grid-cols-12 gap-3">
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 4: ГЛУБОКИЙ АНАЛИЗ — V2 панели (условно)
          ═══════════════════════════════════════════════════════════════════ */}
      {useV2Api && v2Report && (
        <div className="grid grid-cols-12 gap-3">
          {/* Left column */}
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 6: EXPERT — только при expertMode
          ═══════════════════════════════════════════════════════════════════ */}
      {expertMode && (
        <div className="grid grid-cols-12 gap-3">
