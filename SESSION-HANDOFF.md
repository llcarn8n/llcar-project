...
## Часть 4.5. ЧЕСТНАЯ ОЦЕНКА качества KB (важно для S19)

**KB выглядит наполненным, но внутри много шума.** Это результат того, что S17 ETL затащил из `D:/transfer4` много мусора, и patch-за-patch подчистки не решили проблему на корню.

### Типы мусора, который частично остался:
1. **Шаблонные ситуации** — ID с префиксами `yt_` (из YouTube transcripts), `buni_` (из owner manual), `rev_` (из отзывов), `ar_`. Это не диагностические ситуации, а куски другого контента, пролезшие через ETL как situations.
2. **Owner manual warnings** — тексты типа "Запрещается размещать детское автокресло на переднем пассажирском сиденье", "В противном случае возможны смертельные травмы". Часть удалена (438 записей), но остались те, где нет моих ключевых слов (`запрещается`, `травм`, `смертельн`, `в противном случае`).
3. **Mojibake / OCR garbage** — куски текста с перемешанными кириллицей и латиницей, битыми символами. Часть удалена через паттерн `mixed-ocr`.
4. **Generic universal** без конкретики — "Проблемы с зарядкой и стартером", "Общие советы по обслуживанию". Были universal, дублировались ETL во все поколения, частично почищены.
5. **DTC titles на английском** — B0003 "Driver Frontal Stage 3 Deployment Control (Subfault)" и подобное. Мой фильтр `has_cyrillic` не помог, потому что Li Auto source имеет смешанные записи. **Нужен более агрессивный фильтр** или полностью другой источник DTC с русскими переводами.
6. **Пустые OEM артикулы** в parts-catalog.json — 90% имеют `source: "template"` и пустой `part_number`. Это структурный шаблон каталога без реальных данных.

### Чего реально НЕТ на prod (indexed, но сами файлы отсутствуют):
- **107 167 картинок** — только `images.json` индекс. Сами файлы в `D:/transfer4`, не загружены на prod. UI ничего не покажет.
- **210 мануалов** — только `manual_meta.json` со ссылками. Сами `manual.md` (гигабайты) не на prod.

### Почему так получилось:
1. S17 ETL не имел quality gate на входе — копировал всё из source.
2. Я делал patch-за-patch вместо пересборки: -23642 universal, -130 cross-brand, -245 GLM-wrong, -438 warnings. Каждый проход находил новый тип мусора.
3. GLM-верификатор проверил только 3000 из 26093 уникальных ситуаций (12%). Остальные 23000 не проверены.
4. Из verifier findings: 30% записей помечены как `wrong`, 12% как `suspect`, только 47% OK. Экстраполируя — реально качественных ситуаций в KB **~3300 из 7020**, остальные разной степени "шума".

### Правильный путь (S19):
**Не патчить дальше — переделать ETL с quality gate:**
1. При ingestion из source сразу отбрасывать:
   - ID prefixes `yt_`, `buni_`, `rev_`, `ar_`, `warn_`, `safety_`
   - Title/qa без упоминания конкретной brand/model
   - Source field `source: template`, `source: owner_manual`, `source: rev_`
   - Текст с количеством кириллицы меньше 60% от всего текста
2. Для DTC titles — требовать минимум 3 кириллических слова, иначе **отбрасывать**
3. Для parts-catalog — отбрасывать записи с пустыми `part_number` и `source: template` (или помечать как "informational only")
4. Не делать massive universal-duplication при ETL — universal должен оставаться ОДИН раз в `_universal/` и UI cascade к ним
5. Пересобрать KB с нуля из `D:/transfer4` с новым строгим ETL

Оценочный результат после такой пересборки: **~3000-4000 качественных gen-specific ситуаций** (реально relevant diagnostic), **~5000 валидных DTC кодов с русскими описаниями**, **~200 моделей с реальными parts**.

Это лучше чем "7020 ситуаций из которых половина шум".
